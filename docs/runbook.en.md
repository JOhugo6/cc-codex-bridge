...
`inspect` prints the state, journal and paths, including request input and any recorded response. `finish` acquires the conversation lock and completes only local writes for a `received` operation. It checks the expected previous/next state, does not duplicate existing transcript entries, and returns the original result. Repeating it is safe; it never starts Codex. After `finish`, redeliver the same `request_id` to retrieve that result, or continue with a new request. Without `request_id`, take the result printed by `finish` as the completed turn; resending its message would be another turn. Following a process crash, the stale-lock policy can delay recovery by that conversation's lock budget: 35 minutes by default (the 30-minute call budget plus margin), and correspondingly more for a call with a raised `timeout_ms`, up to 65 minutes. Do not remove a live process's lock.

A turn killed at the call ceiling **leaves no partial state**: the state file still holds the last completed turn and the Codex thread stays coherent. That does not make the conversation continuable — `assertComplete` refuses **every** further call while the last operation is anything but `completed`, and `finish` cannot adjudicate a `pending` one. The conversation is therefore closed under that `conversation_id` until an operator resolves it, and a retry belongs in a new conversation as new history. Coherent state means there is nothing to salvage, not that there is a way forward.
...
- **v1 isolation is thread-level only — NOT process/sandbox-level.** In this milestone the bridge backs ALL conversations with ONE shared `codex app-server` process; separation between conversations is logical `conversation_id`/`thread_id` keying, not OS-level process isolation. Each new thread has a validated, pinned cwd, but the sandbox is **`danger-full-access`**, i.e. none: Codex 0.155.1 auto-approves MCP tool calls only under full access, and in any sandboxed mode every such call fails with `MCP tool call requires approval, but approval policy is never`. Cwd pinning is not a filesystem access boundary. Assume Codex behind the bridge can write anywhere and reach the network.
- **User-scope registration.** The bridge command is available across projects and Claude starts its stdio process for the session. Disk continuity is keyed by `conversation_id`; use distinct IDs for unrelated conversations. Only register the bridge on a machine where an unrestricted Codex is acceptable.
...
The live smoke uses existing Codex login, a temporary empty project/state directory, the `danger-full-access` sandbox, two separate bridge/App Server processes and 90-second per-operation limits. It checks remembered random text and the same persisted thread, then verifies native children and supervisors exited and the project stayed empty — emptiness is no longer enforced by the sandbox, only by both prompts writing nothing. It does not change user configuration. Verified on Windows with Node 24.12.0, originally against Codex 0.153.4 and re-run against 0.155.1 after the move to the full-access sandbox; this does not establish Claude relay/Agent Teams behavior or POSIX live compatibility.
